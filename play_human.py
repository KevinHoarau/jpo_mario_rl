...
def play_human(env: gym.Env, callback=None):
    ...
    print("Appuyez D pour avancer et sur O pour sauter.")
    ...
    try:
        while True:
            ...
            if done:
                ...
                if(infos):
                    progress = int(infos["x_pos"]*100/3161)
                    print()
                    print("Vous avez parcouru %d"%progress+"%"+" du niveau")
                    print()
                    print("Comparez-vous à l'IA :")
                    print(pd.read_csv("records.csv"))
                    print()
                    input("Appuyez sur ENTER pour rejouer\n")
